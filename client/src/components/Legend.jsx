const Legend = ({ StageNames, onClickHandler }) => {
  return (
    <div class="inline-flex rounded-md shadow-sm" role="group">

      {StageNames.reduce((all, next) => {
        if (all.indexOf(next.StageName) === -1) {
          all.push(next.StageName);
        }
        return all;
      }, []).map((item) => {
        return (
          <button type="button" class="inline-flex items-center px-4 py-2 text-sm font-medium border-t border-b focus:z-10 focus:ring-2 bg-gray-800 border-gray-600 text-white hover:text-white hover:bg-gray-600 focus:ring-blue-500 focus:text-white"
          onClick={(e) => {
            onClickHandler(e, item);
          }}
          data-stageName={item}
          >
          <svg aria-hidden="true" class="w-4 h-4 mr-2 fill-current" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path d="M5 4a1 1 0 00-2 0v7.268a2 2 0 000 3.464V16a1 1 0 102 0v-1.268a2 2 0 000-3.464V4zM11 4a1 1 0 10-2 0v1.268a2 2 0 000 3.464V16a1 1 0 102 0V8.732a2 2 0 000-3.464V4zM16 3a1 1 0 011 1v7.268a2 2 0 010 3.464V16a1 1 0 11-2 0v-1.268a2 2 0 010-3.464V4a1 1 0 011-1z"></path></svg>
          {item}
        </button>
        );
      })}
</div>
  );
};

export default Legend;

